...
static int expr()
{
	...
	case '9':
		return digit();
	default:
		ERROR("Symbol %c unknown at EXPR\n", token);
		exit(EXIT_FAILURE);
	}
}
...
//	printf("BEEN HERE\n");
	...
	next_token();
	if(token!='='){
		ERROR("Symbol %c unknown at ASSIGN\n", token);
		exit(EXIT_FAILURE);
	}
	next_token();
	int left_reg=expr();
//	int reg=next_register();
	...
	CodeGen(STORE,temp,left_reg,EMPTY_FIELD);
}
...
static void stmt()
{
	switch(token){
		...
		case ';':
			return;
		default:
			ERROR("Symbol %c unknown at STMT\n", token);
			exit(EXIT_FAILURE);
	}
...
static void morestmts()
{
	if(token==';'){
		...
		ERROR("Symbol %c unknown at MORESTMTS\n", token);
		exit(EXIT_FAILURE);
	}	/* YOUR CODE GOES HERE */
}
