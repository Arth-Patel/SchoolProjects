    ...
    if filename:
        try:
            ...
            for line in keyfile_fd:
                ...
                # hash, we may have a valid host/key pair in the keychain BOX in Hashtable
...
class socket:
    ...
    def __init__(self):
		...
		#idk why theres two encryption booleans
    ...
    def connect(self, *args):
        ...
        if self.encrypt:
            #print(args[0][0], str(portTx))
            #print(args[0][0] +  str(args[0][1])) BOX again
            key = (args[0][0], str(portTx))
            if args[0][0] == '127.0.0.1' and ('127.0.0.1', str(portTx)) not in publicKeys:
                #print("inhere")
                key = ('localhost', str(portTx))
            elif args[0][0] == 'localhost' and ('localhost', str(portTx)) not in publicKeys:
                #print("inhere2")
    ...
    def accept(self,*args):
        ...
        syn_ack_packet = self.createPacket(flags=flags, opt=0x0,
                                           sequence_no=self.sequence_no,
        ...
        if self.encrypt:#box
            key = (self.send_address[0],str(self.send_address[1]))
            if self.send_address[0] == '127.0.0.1' and ('127.0.0.1', str(portTx)) not in publicKeys:
                print("inhere")
                key = ('localhost', str(portTx))
            elif self.send_address[0] == 'localhost' and ('localhost', str(portTx)) not in publicKeys:
                print("inhere2")
                key = ('127.0.0.1', str(portTx))
            self.box = Box(privateKeys[('*', '*')], publicKeys[key])
    ...
    def create_data_packets(self, buffer):
        ...
        if self.encrypt:
			newbuffer=self.box.encrypt(buffer,self.nonce)
        else:
            newbuffer=buffer
        # calculates the total packets needed to transmit the entire buffer
        total_packets = len(newbuffer) / MAXIMUM_PAYLOAD_SIZE

        print(str(len(newbuffer)) + " : " + str(len(buffer)))
        ...
        if len(newbuffer) % MAXIMUM_PAYLOAD_SIZE != 0:
            total_packets += 1
            print("Estimated Packets" + str(total_packets))
        ...
        for i in range(0, total_packets):
            ...
            if i == total_packets - 1:
                if len(newbuffer) % MAXIMUM_PAYLOAD_SIZE != 0:
                    payload_len = len(newbuffer) % MAXIMUM_PAYLOAD_SIZE 
                    print("packets" + str(payload_len))

            # creates the new packet with the appropriate header
            if self.encrypt:#OPT
                new_packet = self.createPacket(flags=0x0, opt=0x1,
                                           sequence_no=self.sequence_no,
                                           ack_no=self.ack_no,
                                           payload_len=payload_len)
            else:
			    new_packet = self.createPacket(flags=0x0, opt=0x0,
                                           sequence_no=self.sequence_no,
                                           ack_no=self.ack_no,
                                           payload_len=payload_len)
            ...
            self.data_packets.append(new_packet + newbuffer[MAXIMUM_PAYLOAD_SIZE * i:
    ...
    def send(self, buffer):
        ...
        if self.file_len == -1 and  self.encrypt:
        ...
        # sets the starting sequence number and  data packets starting from this number
    ...
    def recv(self, nbytes):
        ...
        if self.file_len == -1:   
            ...
            #print("filesz" +	str(file_size_packet) )
        ...
        if self.encrypt:#40 bytes for the nonce
            bytes_to_receive+=40
        # also declares a variable to hold all the string of the data that has been received
        data_received = ""
        #print ("vtes to recv " + str(bytes_to_receive) + " bytes")
    ...
    def createPacket(self, flags=0x0, opt =0x0, sequence_no=0x0, ack_no=0x0, payload_len=0x0):
    ...
    def manage_recvd_data_packet(self, packet):
        ...
        ack_packet = self.createPacket(flags=SOCK352_ACK, opt =0x0,
                                       sequence_no=self.sequence_no,
                                       ack_no=self.ack_no)
